...
describe('comment', () => {
  it('must have an author', () => {
    ...
    expect(() => makeComment(comment)).toThrow('El comentario debe teneer un autor.')
  })

  it('must have a valid post id', () => {
    const comment = makeFakeComment({ postId: null })
    expect(() => makeComment(comment)).toThrow('El comentario debe contener un Id de Post.')
  })
  it('must have valid text', () => {
    const comment = makeFakeComment({ text: null })
    expect(() => makeComment(comment)).toThrow(
      'El comentario debe incluir al menos un caracter de texto.'
    )
  })
  it('can be in reply to another comment', () => {
    const comment = makeFakeComment({ replyToId: 'invalid' })
    expect(() => makeComment(comment)).toThrow(
      'Si se provee, el Comentario debe contener un id de respuesta valido.'
    )
  ...
  it('can have an id', () => {
    ...
    expect(() => makeComment(comment)).toThrow('El comentario debe tener un id valido.')
  ...
  it('sanitizes its text', () => {
    ...
    expect(() => makeComment(totallyInsane)).toThrow(
      'El comentario contiene texto no usable.'
    )
  })
